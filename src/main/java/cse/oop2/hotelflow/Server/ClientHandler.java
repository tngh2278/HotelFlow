...
import cse.oop2.hotelflow.Server.service.PaymentService;
...
public class ClientHandler implements Runnable {
    ...
    public ClientHandler(Socket socket, AuthService authService, RoomService roomService) {
        ...
        this.paymentService = new PaymentService("data/payments.csv");
    }
                ...
                try {
                    ...
                    if ("PING".equals(command)) {
                        ...
                        if (userOpt.isPresent()) {
                            ...
                            // OK|ROLE|NAME|PHONE
                            out.println("OK|" + userOpt.get().getRole().name() + "|" + userOpt.get().getName() + "|"
                    ...
                    // 6-1 예약 변경
                    else if ("UPDATE_RESERVATION".equals(command) && parts.length >= 7) {
                        try {
                        ...
                        try {
                            ...
                            Room room = roomList.stream()
                                    .filter(r -> String.valueOf(r.getRoomNum())
                                            .equals(String.valueOf(res.getRoomNum())))
                                    .findFirst()
                            ...
                            for (RoomServiceOrder order : orders) {
                                if (order.getRoomNum() == res.getRoomNum()) {
                                    ...
                                    try {
                                        ...
                                        if (start != -1 && end != -1) {
                                            String priceStr = desc.substring(start + 1, end).replace(",", "")
                                                    .replace(" ", "").trim();
                        ...
                        // 11-2 결제 기록
                    } else if ("PAY_BILL".equals(command) && parts.length >= 6) {
                        // PAY_BILL|reservationId|roomAmount|fnbAmount|totalAmount|method
                        String reservationId = parts[1].trim();
                        int roomAmount = Integer.parseInt(parts[2].trim());
                        int fnbAmount = Integer.parseInt(parts[3].trim());
                        int totalAmount = Integer.parseInt(parts[4].trim()); // 검증용
                        String methodStr = parts[5].trim();

                        try {
                            PaymentMethod method = PaymentMethod.valueOf(methodStr);

                            // 서버 쪽에서 total 다시 계산하여 검증
                            int calcTotal = roomAmount + fnbAmount;
                            if (calcTotal != totalAmount) {
                                out.println("FAIL|금액 합계가 일치하지 않습니다.");
                                return;
                            }
                            ...
                            out.println("OK|" + payment.getId());

                        } catch (IllegalArgumentException e) {
                            out.println("FAIL|잘못된 결제 수단입니다.");
                        } catch (IOException e) {
                            e.printStackTrace();
                            out.println("FAIL|결제 내역 저장 중 오류가 발생했습니다.");
                        }
                    } // 12. 체크아웃
                        ...
                        try {
                            ...
                            List<Reservation> resList = reservationService.getAllReservations();
                            Reservation res = resList.stream()
                                    .filter(r -> r.getId().equals(bookingId))
                                    .findFirst()
                                    .orElseThrow(() -> new IllegalArgumentException("예약을 찾을 수 없습니다."));
                            // 2. 체크아웃 상태인지 확인
                    ...
                    // 19. 기간별 결제 내역 조회 (관리자/직원용)
                        ...
                        try {
                            ...
                            for (Payment p : payments) {
                                ...
                                boolean inRange = (payDate.isEqual(fromDate) || payDate.isAfter(fromDate))
                                        && (payDate.isEqual(toDate) || payDate.isBefore(toDate));
                    ...
                    HotelLogger.error(
                            "ClientHandler",
                            "요청 처리 중 예상치 못한 오류: " + line,
                            e);
                    out.println("FAIL|서버 내부 오류가 발생했습니다.");
                }
            }
        } catch (IOException e) {
            HotelLogger.error(
